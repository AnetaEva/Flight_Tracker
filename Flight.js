...
export default class Flight{
    ...
    const popupElement = `
      <div class="popup-callsign-container">
        ...
        <div class="popup-callsign-right">
          <p>•${this.callsign}</p>
          <span class="stat-label"><p id=${this.callsign}-altitude>A:${this.getAltitude()}</p> <p id="${this.callsign}-bearing">H:${this.getBearing()}</p> <p id="${this.callsign}-speed">S:${this.getSpeed()}</p></span>
        </div>
      </div>
    `;
  ...
  select() {
    ...
    document.getElementById('big-callsign').innerHTML = this.getCallsign();
...
/* THIS METHOD CHANGES THE INFORMATION INSIDE LOADED FOR SCENARIO */
  getFlightName() {
    //return `Flight ${this.getCallsign()} O: ${this.getOrigin()} D:${this.getDestination()}`;// return `Flight number ${this.getId()} O: ${this.getOrigin()} D:${this.getDestination()}`;
    return `Flight ${this.getCallsign()}`;
  }
  
/*THIS METHOD IS HOW THE FLIGHT NAMES GET GENERATED INSTEAD OF A JSON OR XML FILE*/
  static genCallsign() {
    let sign = 'N';
  ...
  takeOff() {
    ...
    let radAngle = Math.atan2(normalizedY, normalizedX); //Angle from primary x axis in radians
    let degAngle = radAngle * (180 / Math.PI); //Angle in degrees
    this.marker.setRotationAngle(180);
    ...
    if (nextTickPos.distanceTo(this.destination) > currentCoords.distanceTo(this.destination)) { // if overshoot, don't
      ...
      this.setBearing(Number(Flight.calculateBearingFromPoints(currentCoords, nextTickPos)));
    }
  }
  ...
  onTick(tick) {
    ...
    switch (this.getAction()) {
      case 'fly': {
        ...
        this.setBearing(this.getBearing);
        break;
      }
      case 'bearing': {
      ...
      case 'landed': {
        if (!this.arrived) {
          this.arrived = true;
          console.log(`Flight ${this.getCallsign()} has landed`);
        }
        break;
      }
      case 'pretakeoff': {
    ...
    if (this.selected) {
      ...
      document.getElementById(`big-bearing`).innerHTML = this.getBearing();
    }
    ...
    document.getElementById(`${this.getCallsign()}-bearing`).innerHTML = 'H:' + this.getBearing();
  }
}